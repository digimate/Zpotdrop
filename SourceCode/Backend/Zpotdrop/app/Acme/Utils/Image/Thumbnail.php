...
* @author Emilio Rodriguez <hide@address.com>
...
class Thumbnail {
	...
	function __construct($srcFile) 	{
        ...
        switch( $imgSize[2] ){
			case 2:
				...
				$this->img["src"] = ImageCreateFromJPEG ($srcFile);
			break;
			case 3:
				//PNG
				$this->img["format"]="PNG";
				$this->img["src"] = ImageCreateFromPNG ($srcFile);
			break;
			default:
		...
		$this->output_format = $this->img["format"];
		$this->img["x"] = $imgSize[0];  //original dimensions
		$this->img["y"] = $imgSize[1];
    ...
    **/
    function process () {
        ...
        $X_des = (int)$this->img["x_thumb"];
        $Y_des = (int)$this->img["y_thumb"];
    ...
    **/
    function CalculateQFactor($size)  {
        //based on: JPEGReducer class version 1,  25 November 2004,  Author: huda m elmatsani, Email :hide@address.com
